...
Build frontend lokal membutuhkan Node.js 20.19+, 22.12+, atau versi yang lebih baru. Server hanya memerlukan PHP CLI (bukan `php-cgi`), Composer, dan perintah standar `cp`; `rsync` tidak diperlukan. Script mendeteksi lokasi PHP CLI cPanel dan CloudLinux yang umum. Jika binary PHP atau Composer berada di lokasi lain, berikan `PHP_BIN` dan `COMPOSER_BIN` saat menjalankan `deploy.sh`.
