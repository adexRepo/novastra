            ...
            <div>
                ...
                @if ($order->status === 'COMPLETED')
                    <form class="card mt-6 p-5" method="post" action="{{ route('orders.feedback', $order) }}">
                        @csrf
                        <h2 class="font-display text-2xl">Bagaimana pesanan Anda?</h2>
                        ...
                        <textarea class="field py-3" name="message" placeholder="Ceritakan pengalaman Anda" required></textarea>
                        <button class="btn mt-4">Kirim masukan</button>
                    </form>
                @endif
            </div>
